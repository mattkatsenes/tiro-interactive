...
<!-- Lines -->
	<xsl:template match="l">
		...
		<xsl:variable name="line_number-real">
			<xsl:value-of select="@n" />
		</xsl:variable>
		<span class="l" id="LN-{$line_number-real}">
			<xsl:apply-templates/>
		</span>
		<br />
	</xsl:template>
...
<!-- Glossed Terms -->
	...
	<xsl:template name="entry" match="entry">
		...
		<xsl:if test="//l[contains($term_numbers,@n)]">
			<xsl:variable name="numberofentries">
				<xsl:value-of select="count(//l[contains($term_numbers,@n)])"/>
			</xsl:variable>
