...
BEGIN
	SELECT RAISE(ROLLBACK, 'insert on table studentUsers violates foreign key constraint.') WHERE new.teacher_id IS NULL OR (SELECT username FROM teacherUsers WHERE username = NEW.teacher_id) IS NULL;
END;
